...
- **分类（marketplace 浏览）**：skills 表**新增**原生 `category` 字段（上游无此字段；其 `capabilityTags` 是发布时正则派生的**安全风险标签**——`convex/lib/skillCapabilityTags.ts`，封闭词汇表 9 值，不可作浏览分类）；plugin 保留 `pluginCategory` 字段但**停用关键词派生**（`derivePluginCategoryTags` 的 signals 匹配，词汇表为英文技术分类）。两者均改为**发布时由运营显式指定**，词汇表由运营在 fork 自定义（业务分类、有序）。`category` 字段与 `/skills|plugins/categories` 端点见 skill-plugin-lifecycle.md §3.1b
